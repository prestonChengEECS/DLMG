...
public class PlayerInventory : MonoBehaviour
{
    ...
    public bool AddCollection(PickupItem item)
    {
        for (int i = 0; i < collections.Count; i++)
        {
            ...
            {
                collections[i] = item;
                GameObject expanded = item.collectibleExpanded;
                InventoryUI.instance.AddItemToCollections(item.icon, item.itemName, item.itemDescription, expanded);
                return true;
            }
        }
        return false;
